...
import { Frown, Meh, Smile, SmilePlus, Zap, ArrowLeft, ArrowRight, SkipForward, Check, Utensils, Clock, MapPin, Gauge, Heart, AlertTriangle, StickyNote, Layers, ListChecks } from 'lucide-react'
...
// Steps for "quick flow" mode
const QUICK_STEPS = [
  { id: 'name', title: 'What did you eat?', icon: Utensils, required: true },
  { id: 'time', title: 'When?', icon: Clock, required: true },
  { id: 'mode', title: 'How do you want to fill this in?', icon: Layers, required: true },
]
// Steps after mode selection for quick flow
const DETAIL_STEPS = [
...
export default function LogMeal() {
  ...
  const [mode, setMode] = useState(null) // 'swipe' | 'quick' | null
  const [saving, setSaving] = useState(false)
  const [toast, setToast] = useState(null)
  const [done, setDone] = useState(false)
  const [suggestionsApplied, setSuggestionsApplied] = useState(false)
  const [swipePhase, setSwipePhase] = useState(0) // 0=context, 1=prep, 2=triggers, 3=supports, 4=done
  ...
  useEffect(() => {
    if (step === 0 || (mode === 'quick' && currentStepDef?.id === 'notes')) {
      setTimeout(() => inputRef.current?.focus(), 400)
    }
  }, [step, mode])

  // Apply smart pre-selections when entering quick flow
  useEffect(() => {
    if (mode === 'quick' && !suggestionsApplied && form.meal_name.trim()) {
  ...
  // ── Steps logic ──
  // Before mode selection: steps 0-2 (name, time, mode)
  // After mode selection (quick): detail steps 0-4
  const isPreMode = mode === null
  const allSteps = isPreMode ? QUICK_STEPS : DETAIL_STEPS
  const detailStep = isPreMode ? 0 : step
  const currentStepDef = allSteps[step]
  const totalSteps = isPreMode ? QUICK_STEPS.length + DETAIL_STEPS.length : DETAIL_STEPS.length
  const progressStep = isPreMode ? step + 1 : QUICK_STEPS.length + step + 1
  const progressTotal = QUICK_STEPS.length + DETAIL_STEPS.length
  ...
  }, [step, allSteps.length, isPreMode])
  ...
  const swipeDecks = [
    { phase: 0, title: 'Where did you eat?', cards: ALL_CONTEXTS.map((c) => ({ id: `ctx-${c}`, label: c, category: 'Context', suggested: suggestions.contexts.includes(c) })) },
    { phase: 1, title: 'How was it prepared?', cards: ALL_PREP_STYLES.map((p) => ({ id: `prep-${p}`, label: p, category: 'Preparation', suggested: suggestions.preps.includes(p) })) },
    { phase: 2, title: 'Any triggers?', cards: triggerOptions.map((t) => ({ id: `trig-${t}`, label: t, category: 'Trigger', suggested: false })) },
    { phase: 3, title: 'Take anything for it?', cards: ALL_SUPPORTS.map((s) => ({ id: `sup-${s}`, label: s, category: 'Support', suggested: false })) },
  ].filter((d) => d.cards.length > 0)

  const handleSwipeDeckComplete = (selected) => {
    const deck = swipeDecks[swipePhase]
    if (!deck) return

    if (deck.phase === 0) setForm((f) => ({ ...f, contexts: selected }))
    else if (deck.phase === 1) setForm((f) => ({ ...f, preparation_styles: selected }))
    else if (deck.phase === 2) setForm((f) => ({ ...f, triggers: selected }))
    else if (deck.phase === 3) setForm((f) => ({ ...f, supports: selected }))

    if (swipePhase < swipeDecks.length - 1) {
      setSwipePhase((p) => p + 1)
    } else {
      // Swipe decks done — move to portion/reaction/notes in quick flow
      setMode('quick')
      setForm((f) => ({ ...f, portion_size: 'regular' }))
      setStep(1) // portion step in DETAIL_STEPS
    }
  }
  ...
  if (mode === 'swipe' && swipePhase < swipeDecks.length) {
    const deck = swipeDecks[swipePhase]
    const swipeProgress = QUICK_STEPS.length + swipePhase + 1
    return (
      <div className="min-h-[70vh] flex flex-col">
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <ProgressRing step={swipeProgress} total={progressTotal} />
        </div>
        ...
        <div className="card-flow-enter">
          ...
          <SwipeDeck
            key={`deck-${swipePhase}`}
            cards={deck.cards}
            onComplete={handleSwipeDeckComplete}
          />
        </div>
  ...
  // ── Quick Flow / Pre-mode Render ──
  const isLast = !isPreMode && step === allSteps.length - 1
  const cardKey = `step-${step}-${direction}-${mode}`
  ...
  return (
    <div className="min-h-[70vh] flex flex-col">
      ...
      {/* Top bar */}
      <div className="flex items-center justify-between mb-6">
        <button type="button" onClick={step === 0 && isPreMode ? () => navigate('/home') : goBack}
          className="flex items-center gap-1.5 text-sm font-medium text-stone hover:text-terracotta transition-colors cursor-pointer p-1">
          <ArrowLeft className="w-4 h-4" />
          {step === 0 && isPreMode ? 'Cancel' : 'Back'}
        </button>
        <ProgressRing step={progressStep} total={progressTotal} />
      </div>
        ...
        <div className={direction === 'forward' ? 'card-flow-enter' : 'card-flow-enter-back'}>
          ...
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              ...
              {currentStepDef?.required && currentStepDef.id !== 'mode' && (
                <span className="text-[11px] font-semibold uppercase tracking-widest text-terracotta">Required</span>
              )}
            </div>
          </div>
          ...
          <div className="flex-1">
            ...
            {isPreMode && currentStepDef.id === 'name' && (
            ...
            {isPreMode && currentStepDef.id === 'time' && (
              <div className="space-y-4">
                <button type="button"
            ...
            {isPreMode && currentStepDef.id === 'mode' && (
              <div className="space-y-3">
                <button type="button" onClick={() => selectMode('swipe')}
                  className="option-bounce w-full flex items-center gap-4 p-5 bg-white/60 backdrop-blur-sm rounded-2xl border-2 border-sand/40 hover:border-terracotta/30 hover:shadow-md transition-all cursor-pointer text-left">
                  <div className="w-12 h-12 rounded-2xl bg-terracotta/10 flex items-center justify-center shrink-0">
                    <Layers className="w-6 h-6 text-terracotta" strokeWidth={1.8} />
                  </div>
                  <div>
                    <span className="block font-semibold text-bark">Swipe mode</span>
                    <span className="block text-sm text-stone font-light mt-0.5">Swipe right to include, left to skip. One-handed, fast.</span>
                  </div>
                </button>

                <button type="button" onClick={() => selectMode('quick')}
                  className="option-bounce w-full flex items-center gap-4 p-5 bg-white/60 backdrop-blur-sm rounded-2xl border-2 border-sand/40 hover:border-sage/30 hover:shadow-md transition-all cursor-pointer text-left">
                  <div className="w-12 h-12 rounded-2xl bg-sage/10 flex items-center justify-center shrink-0">
                    <ListChecks className="w-6 h-6 text-sage" strokeWidth={1.8} />
                  </div>
                  <div>
                    <span className="block font-semibold text-bark">Tap mode</span>
                    <span className="block text-sm text-stone font-light mt-0.5">Tap chips to select. Smart pre-fills based on your meal.</span>
                  </div>
                </button>
              </div>
            )}

            {/* ── DETAIL STEPS (quick/tap mode) ── */}

            {!isPreMode && currentStepDef?.id === 'context' && (
              <div className="space-y-6">
                <div>
                  <p className="text-[13px] font-semibold uppercase tracking-wide text-stone mb-1">Where?</p>
                  <p className="text-xs text-stone-light mb-3">Pre-filled for "{form.meal_name}" — adjust if needed</p>
            ...
            {!isPreMode && currentStepDef?.id === 'portion' && (
            ...
            {!isPreMode && currentStepDef?.id === 'reaction' && (
            ...
            {!isPreMode && currentStepDef?.id === 'triggers' && (
              <div className="space-y-6">
            ...
            {!isPreMode && currentStepDef?.id === 'notes' && (
              <div>
      ...
      {currentStepDef?.id !== 'mode' && (
        <div className="flex items-center gap-3 mt-8 pb-4">
          {!isPreMode && !currentStepDef?.required && (
          ...
          <button type="button"
            ...
            disabled={(isPreMode && step === 0 && !form.meal_name.trim()) || saving}
